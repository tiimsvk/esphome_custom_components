...
void IP5306::setup() {
  ...
  uint8_t sys0, sys1, chg0, chg1, chg2, sys2;
  ...
  // --- Nacitanie a nastavenie Selectov (Oprava zobrazovania) ---
  
  // 1. Load Shutdown Time (SYS_CTL2, bits 3:2)
  ...
  // 2. Charge Cutoff Voltage (CHARGER_CTL1, bits 1:0)
  ...
  // 3. Charge Termination Current (CHARGER_CTL2, bits 3:2)
...
void IP5306::update() {
  ...
  if (this->read_register(IP5306_REG_READ0, &status_0, 1) == i2c::ERROR_OK) {
    bool vin_present = status_0 & 0x20;
    bool is_full = vin_present && (status_0 & 0x10);

    if (this->charger_connected_ != nullptr) {
      if (this->first_update_ || this->last_charger_connected_ != vin_present) {
        this->charger_connected_->publish_state(vin_present);
        this->last_charger_connected_ = vin_present;
      }
    }
  ...
  if (this->current_sensor_ != nullptr) {
    ...
    if (this->read_register(IP5306_REG_READ1, &data_curr, 1) == i2c::ERROR_OK) {
      ...
      // Maly prah (0.01A) na zabranenie spamovaniu pri jemnych zmenach
...
void IP5306Select::control(const std::string &value) {
  ...
  // Manualne zistenie indexu, pretoze helpers nemusia byt dostupne
  int index = 0;
